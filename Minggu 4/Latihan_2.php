...
class Baju extends ItemProduk {
    ...
    public function Ukuran() {
        $size = "Custom";
        if ($this->getUkuran() > 40 && $this->getUkuran() <= 45){
            $size = "S";
        } elseif ($this->getUkuran() > 45 && $this->getUkuran() <= 48) {
            $size = "M";
        } elseif ($this->getUkuran() > 48 && $this->getUkuran() <= 53) {
            $size = "L";
...
$topi = new Topi("Converse", 50, "Putih", "Light");
echo $topi->getInfoProduk();

echo "<br><br>";

$celana = new Celana("Calvin Klein", 32, "Hitam", "Panjang", "Jeans");
echo $celana->getInfoProduk();

echo "<br><br>";

$baju = new Baju("Supreme", 52, "Merah", "Kaos");
